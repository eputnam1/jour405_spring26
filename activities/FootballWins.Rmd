...
The low p-value indicates the model is statistically significant, and the relatively high Multiple R-squared indicates that the model is a good fit, so the model is useful when it comes to assessing the correlation of these elements. However, it cannot be used to make a very believable win total prediction, given the range of residuals.
...
Points scored has the most impact on wins. Yards gained has the least impact on wins, though still somewhat impactful. Outside of wins, takeaways did not impact yards gained at all, which makes sense. Takeaways did not have a very strong impact on points scored either.
...
This model could be used to cover predictions regarding a team's game (which team has more points scored, less turnovers, more wins already is more likely to win), a certain part of the team's performance (Does the other team have a lot of turnovers? This likely impacts their opponents' offense), or, more broadly, the team's season (how do all these factors relate to wins?).
...
This model's Multiple R-squared is higher than the previous, given that yards was removed and replaced with giveaways. 
...
We saw before that yards had less total impact (0.4430188) on wins than giveaways have (-0.5833568).
