...
There appears to be correlation between unemployment and turnout, education and income, percentage white and turnout, and percentage white and population. There seems to be almost zero correlation between median age and education.

The unemployment, percentage white, income, and median age scatterplots appear to have a linear relationship with the dependent variable, while education and population do not.
...
The Multiple R-squared tells us that over 85% of the variability in turnout is explained by the model. The Adjusted R-squared, accounting for the number variables, was 0.8023, which signifies a pretty strong fit. Percentage white, percentage unemployed, and percentage with a bachelor's degree or higher are the most significant, while population and income are the least.
...
I did not make a model as strong as the original, in fact the original has an Adjusted R-squared that is 0.2 greater than the last model I made, Model 4. My thought was to remove the two least correlated categories and try with the others. I did not include percentage white in Model 4, which may have led to this.

```{r}
# Model 2
model2 <- lm(turnout ~ pct_bachelors_higher + pct_unemployed + pct_white, data = md_turnout)
summary(model2)

# Model 3
model3 <- lm(turnout ~ pct_bachelors_higher + pct_unemployed + pct_white + median_age, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ pct_bachelors_higher + pct_unemployed + median_age, data = md_turnout)
summary(model4)
...
These models help us to assess what demographic categories have a direct correlation with higher voting percentage across Maryland counties. Looking at the first model, there is a negative correlation between having a bachelor's degree or higher and voter turnout in Maryland, which seems unusual. This could be an angle. Percentage unemployed has an even stronger negatuve correlation, which could also be worth looking into.
