...
spotify_songs |>
  ggplot() +
  geom_histogram(aes(x = danceability), bindwidth = 2) +
...
Did mean and sd in last task, here is histogram with just mean:
...
I would not call this dataset normal, although the mean does fall close to the peak of the curve, because it appears skewed right on the graph. The distribution tells us that most of the top Spotify songs have a danceability of 0.5 or higher. Simply put, most of the top songs on Spotify are danceable.
