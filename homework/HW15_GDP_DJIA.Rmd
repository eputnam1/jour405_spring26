...
```{r}
stock_data <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/gdp_djia.csv")
```
...
```{r}
stock_data |> summarize(correlation = cor(DJIA, GDP, method="pearson"))
```
...
There is a strong correlation between the DJIA and GDP variables, as evidenced both by the correlation calculation and the scatterplot. The correlation calculation resulted in 0.95, a high correlation. The scatterplot shows a very defined positive correlation, with a small margin of error. Thus, it can be determined that the GDP is positively impacted by a strong stock market.
