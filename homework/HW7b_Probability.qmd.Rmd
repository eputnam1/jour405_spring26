...
wh_paid |> summarise(meansal = mean(SALARY), mediansal = median(SALARY), sdsal = sd(SALARY))
...
wh_salaries |>
  ggplot() +
  geom_histogram(aes(x = SALARY), bindwidth = 2) +
...
The distribution appears normal. Most salaries are clustered in the middle of the graph, between about $70,000 and $125,000. As expected, there are fewer high salaries than their are lower salaries.
...
The data follows the empirical rule very closely, within 3-4 percentage points. More deviation from the expected percentages means the data is less and less normally distributed.
...
There is an approximately 16.9% chance that a randomly selected White House employee makes more than $150,000. Whereas, the normal approximation gives you the likelihood that a randomly selected employee makes less than $150,000.
...
salary80 <- 80000
z_score <- (salary80 - mean_sal) / sd_sal
...
salary120 <- 120000
z_score <- (salary120 - mean_sal) / sd_sal
...
Approximately 36.6% of White House salaries fall between $80,000 and $120,000. This tells us that White House salaries are tiered as would be expected: the approximate middle third makes between $80k and $120k, while the rest fall above and below that range. About 78.1% of employees make over $80k, and about 41.5% make less than $120k, so we can determine that there are more people with higher salaries than this range than there are people will lower salaries.
...
Reality differs from the prediction in that salaries are not always perfectly distributed, and are spread out based on differing jobs and what they entail. The prediction anticipated many more salaries below $60k, but this does not account for the importance of White House jobs, which could be a significant factor responsible for driving up salaries. Additionally, the observed count was higher than the prediction for salaries above $150k, which could be worth looking into. This could be explained by the fact that many White House employees are selected by the administration, and salaries can be determined individually based on what that administration deems to be what the job is worth.
...
Headline: What Are the White House's Highest Paid Employees Doing?

Lede: White House employees making more than $120,000 per year are in the majority. White House salary data shows that approximately 41.5% of White House employees make less than $120,000 per year. This raises the question: who is reaping the benefits of these salaries, and why?
